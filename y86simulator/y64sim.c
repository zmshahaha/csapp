...
cc_t compute_cc(alu_t op, long_t argA, long_t argB)
{
    ...
    bool_t ovf = (!op)&&(!(((int)argA>0)^((int)argB>0)))&&(((int)val>0)^((int)argA>0))||op&&(((int)argA>0)^((int)argB>0))&&(!(((int)val>0)^((int)argA>0)));
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
   bool_t doit = FALSE;
    ...
    switch(cond){
      ...
      case 1: doit=(SF^OF)|ZF;break;
      case 2: doit=SF^OF;break;
      case 3: doit=ZF;break;
      case 4: doit=!ZF;break;
      case 5: doit=!(SF^OF);break;
      case 6: doit=!(SF^OF)&!ZF;break;
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if(icode==2||icode==3||icode==4||icode==5||icode==6||icode==10||icode==11){
        ...
        regB=GET_REGB(reg);
        if(NORM_REG(regA)){
             get_long_val(sim->r, 8*regA, &regA_val);
        }
        if(NORM_REG(regB)){
             get_long_val(sim->r, 8*regB, &regB_val);
        }
        next_pc++;
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVQ: /* 2:x regA:regB */
            if(cond_doit(sim->cc, ifun))
                set_long_val( sim->r, 8*regB, regA_val);
            sim->pc = next_pc;
            break;
      case I_IRMOVQ: /* 3:0 F:regB imm */
            set_long_val( sim->r, 8*regB, imm);
            sim->pc = next_pc;
            break;
      case I_RMMOVQ: /* 4:0 regA:regB imm */
            if (imm+regB_val<0||imm+regB_val+8>MEM_SIZE) {
                err_print("PC = 0x%lx, Invalid data address 0x%lx",sim->pc, imm+regB_val);
                return STAT_ADR;
            }
            set_long_val( sim->m, imm+regB_val, regA_val);
            sim->pc = next_pc;
            break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
           if (!get_long_val( sim->m, imm+regB_val, &data)) {
               err_print("PC = 0x%lx, Invalid data address 0x%lx",sim->pc, imm+regB_val);
               return STAT_ADR;
            }
            set_long_val( sim->r, 8*regA, data);
            sim->pc = next_pc;
            break;
      case I_ALU:/* 6:x regA:regB */
            sim->cc=compute_cc(ifun, regA_val, regB_val);
            regB_val=compute_alu(ifun, regA_val, regB_val);
            set_long_val(sim->r,8*regB,regB_val);
            sim->pc = next_pc;
            break;
      case I_JMP: /* 7:x imm */
            ...
            break;
      case I_CALL: /* 8:0 imm */
            get_long_val(sim->r,8*REG_RSP,&stack);
            if (stack<0||stack>=MEM_SIZE) {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
                return STAT_ADR;
            }
            set_long_val(sim->r,8*REG_RSP,stack=stack-8);
            if (stack<0||stack>=MEM_SIZE) {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
                return STAT_ADR;
            }
            set_long_val(sim->m,stack,next_pc);
      ...
      case I_RET: /* 9:0 */
            get_long_val(sim->r,8*REG_RSP,&stack);
            set_long_val(sim->r,8*REG_RSP,stack+8);
            if (!get_long_val(sim->m,stack,&data)) {
                err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
            sim->pc=data;
      ...
      case I_PUSHQ:/* A:0 regA:F */    
            ...
            set_long_val(sim->m,stack,regA_val);
            sim->pc = next_pc;
            break;
      case I_POPQ: /* B:0 regA:F */
    	    get_long_val(sim->r,8*REG_RSP,&stack);
    	    if (!get_long_val(sim->m,stack,&regA_val)) {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
                return STAT_ADR;
            }
            set_long_val(sim->r,8*REG_RSP,stack+8);
            set_long_val(sim->r,8*regA,regA_val);
            sim->pc = next_pc;
    	    break;
      default:
