...
void set_n_val(byte_t *dest,long long val, int bytes)
{
...
{
    for(symbol_t *ptr=symtab;ptr;ptr=ptr->next)
    {
        int slen=strlen(ptr->name);
        if (strncmp(ptr->name, name,slen) == 0)
	    return ptr;
    }
    return NULL;
}
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    SKIP_BLANK(*ptr);
    /* find register */
    if(!find_register(*ptr))
    {
		err_print("Invalid REG")
		return PARSE_ERR;
	}
    *regid=(find_register(*ptr))->id;
    *ptr=*ptr+(reg_table[*regid].namelen);
    return PARSE_REG;
    /* set 'ptr' and 'regid' */
}
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    for(int k=0;k<i;++k)(*name)[k]=(*ptr)[k];
    (*name)[i]='\0';
    (*ptr)=(*ptr)+i;
    if(i>0)
        return PARSE_SYMBOL;
...
parse_t parse_digit(char **ptr, long long *value)
{
    ...
    if(IS_DIGIT(*ptr))
    {
        ...
        if(((*ptr)[0]=='0'&&(*ptr)[1]=='x'))
        {
            *value=sign*strtoull(*ptr,ptr,16);
            return PARSE_DIGIT;
        }
        else
        {
            *value=sign*strtoull(*ptr,ptr,10);
            return PARSE_DIGIT;
        }
    }
    /* set 'ptr' and 'value' */
...
parse_t parse_mem(char **ptr, long long *value, regid_t *regid)
{
    ...
    parse_t isdigit=parse_digit(ptr,value);
    if(isdigit==PARSE_ERR)*value=0;
    if(*ptr[0]=='(')
    {
        (*ptr)++;
        if(parse_reg(ptr,regid)==PARSE_ERR||*ptr[0]!=')')/*PTR IS CHANGED!!*/
        {
 ...
 */
parse_t parse_label(char **ptr, char **name)//symtable
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    /* allocate name and copy to it */
    int i;
    for(i=0;(*ptr)[i]!='\0'&&(*ptr)[i]!='\n'&&(*ptr)[i]!=' '&&(*ptr)[i]!=':';++i);
    if((*ptr)[i]==':')
    {
...
*           call SUM  #invoke SUM function */
    ...
    if(IS_END(y64asm)||IS_COMMENT(y64asm)){
        return line->type;
    }
        
    /* is a comment ? */
    /* is a label ? */
    parse_t islabel=parse_label(&y64asm,&label);
    if (islabel==PARSE_LABEL)
        line->type=TYPE_INS;
    
    if(islabel==PARSE_LABEL&&add_symbol(label)<0)
    {
    ...
    /* is an instruction ? */
    parse_t isinst=parse_instr(&y64asm,&inst);
    /* set type and y64bin */
    line->y64bin.addr = vmaddr;
    if(isinst==PARSE_INSTR)
    {
        ...
        if (HIGH(inst->code) != I_DIRECTIVE) {
            ...
            switch (HIGH(inst->code))
            {
                ...
                case I_RRMOVQ:
                case I_ALU:
                    if (parse_reg(&y64asm, &regA) == PARSE_ERR ||parse_delim(&y64asm, ',') == PARSE_ERR ||parse_reg(&y64asm, &regB) == PARSE_ERR)
                    {
					    line->type = TYPE_ERR;
					    return line->type;
				    }
                ...
                case I_IRMOVQ:
                {
                    parse_t parimm=parse_imm(&y64asm, &symbol, &val);
					if (parimm== PARSE_ERR ||parse_delim(&y64asm, ',') == PARSE_ERR ||parse_reg(&y64asm, &regB) == PARSE_ERR) 
                    {
						line->type = TYPE_ERR;
						return line->type;
					}
					line->y64bin.codes[1] = HPACK(REG_NONE, regB);
					if (parimm == PARSE_DIGIT) 
						set_n_val(&(line->y64bin.codes[2]), val, 8);
                    else 
						add_reloc(symbol, &line->y64bin);
					break;
                }
                case I_RMMOVQ:
				    if (parse_reg(&y64asm, &regA) == PARSE_ERR ||parse_delim(&y64asm, ',') == PARSE_ERR ||parse_mem(&y64asm, &val, &regB) == PARSE_ERR) 
                    {
					    line->type = TYPE_ERR;
					    return line->type;
				    }
				    ...
				    break;
                case I_MRMOVQ:
				    if (parse_mem(&y64asm, &val, &regB) == PARSE_ERR ||parse_delim(&y64asm, ',') == PARSE_ERR ||parse_reg(&y64asm, &regA)) 
                    {
					    line->type = TYPE_ERR;
					    return line->type;
				    }
			    ...
			    case I_JMP:
			    case I_CALL:
				    if (parse_symbol(&y64asm, &symbol) == PARSE_ERR)
                    {
            ...
            {
                case D_POS:
					if (parse_digit(&y64asm, &val) == PARSE_ERR) {
						line->type = TYPE_ERR;
						return line->type;
					}
					vmaddr = val;
				...
				case D_DATA: 
					{
						...
						parse_t pt = parse_data(&y64asm, &symbol, &val);
						if (pt == PARSE_DIGIT) {
